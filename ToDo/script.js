...
function taskHTML(t) {
  return `<div class="task-item ${t.done ? 'done' : ''}" id="task-${t.id}">
    <div class="check-box ${t.done ? 'checked' : ''}" data-id="${t.id}" role="checkbox" aria-checked="${t.done}" tabindex="0">
      <svg class="check-icon" viewBox="0 0 10 10" fill="none" xmlns="http://www.w3.org/2000/svg">
...
function render() {
  ...
  if (visible.length === 0) {
    container.innerHTML = `<div class="empty-state">${filter === 'done' ? 'no completed tasks yet' :
